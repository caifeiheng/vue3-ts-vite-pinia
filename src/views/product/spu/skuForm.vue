...
import { getSpuImg, getSaleAttr,addSku } from '@/api/product/spu';
...
const initSkuData = async (c1Id: number | string, c2Id: number | string, spu: any) => {
  ...
  //获取平台属性
  let result: any = await getAttrData(c1Id, c2Id, spu.category3Id);
  //获取对应的销售属性
  let result1: any = await getSaleAttr(spu.id);
  //获取照片墙的数据
  let result2: any = await getSpuImg(spu.id);
  //平台属性
...
//引入请求API
import { getAttrData } from '@/api/product/attr';
import { getSpuImg, getSaleAttr, reqAddSku } from '@/api/product/spu';
import type { SkuData } from '@/api/product/spu/type'
import { ElMessage } from 'element-plus';
import { ref, reactive } from 'vue';
//平台属性
