          ...
          <el-table border style="margin:10px 0px" :data="saleAttr">
                ...
                <el-table-column label="销售属性值">
                    ...
                    <template #="{ row, $index }">
                        ...
                        <el-input ref="getFocus" @blur="toLook(row)" v-model="row.saleAttrValue" v-show="row.flag"
                            placeholder="请你输入属性值" size="small" style="width:100px"></el-input>
                        <el-button @click="toEdit(row)" v-show="!row.flag" type="primary" size="small" icon="Plus"></el-button>
                    </template>
                </el-table-column>
...
//引入获取数据的接口
import {getAllTradeMark,getSpuImg,getSaleAttr,getAllSaleAttr,addOrAmendSpu} from '@/api/product/spu/index'
//引入vue
import {ref,computed,nextTick} from 'vue'
//引入spu数据类型
import type {SaleAttrValue,AllSaletAttr,SaleAttr,SpuImg,TradeMarkData,SpuData,GetAllTradeMark,SpuHasImg,SaleAttrResponseData,AllSaletAttrResponseData} from '@/api/product/spu/type'
//引入elementplus
...
//全部销售属性
let allSaleAttr = ref<AllSaletAttr[]>([]);
...
const getSpu = async (spu:SpuData)=>{
  ...
  //SPU对应商品图片
  imgList.value = imgurl.data.map(item => {
      return {
          name: item.imgName,
          url: item.imgUrl
      }
  ...
  //存储已有的SPU的销售属性
  saleAttr.value = sellAttrData.data;
  //存储全部的销售属性
  allSaleAttr.value = allSellAllr.data;
...
const save = async () => {
  ...
  let result = await addOrAmendSpu(SpuParams.value);
...
const initAddSpu = async (c3Id: number | string) => {
  ...
  //获取全部品牌的数据
  let result: GetAllTradeMark = await getAllTradeMark();
  let result1: AllSaletAttrResponseData = await getAllSaleAttr();
  //存储数据
...
//对外暴露获取数据的方法等父组件调用
 defineExpose({getSpu,initAddSpu})
</script>
<style scoped></style>