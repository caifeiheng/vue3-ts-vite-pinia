...
//引入获取数据接口
import {getMenuData,addOrUpdateMenu,deleteMenu} from '@/api/acl/menu/index'
//引入数据类型
import type {RoleMenuResponseData,RoleMenuChild,AddOrUpdateMenu} from '@/api/acl/menu/type'
import { ElMessage } from 'element-plus'
onMounted(()=>{
  getMenu()
})
let PermisstionArr = ref<RoleMenuChild>([])
let dialogVisible = ref<boolean>(false)
let menuData = reactive<AddOrUpdateMenu>({
  code:'',
  id: 0,
  level: 0,
  name: '',
...
const getMenu = async()=>{
  let result:RoleMenuResponseData =  await getMenuData()
  if(result.code == 200){
...
const save = async () => {
  ...
  let result: any = await addOrUpdateMenu(menuData);
  if (result.code == 200) {
      //对话框隐藏
...
//添加菜单按钮的回调
const addPermisstion = (row: AddOrUpdateMenu) => {
  //清空数据
...
//编辑已有的菜单
const updatePermisstion = (row: AddOrUpdateMenu) => {
  dialogVisible.value = true;
