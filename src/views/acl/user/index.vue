<template>
  ...
  <el-card style="margin: 10px 0px;">
        ...
        <el-button type="primary" size="default" :disabled="selectIdArr.length ? false : true"
            @click="onDeleteSelectUser">批量删除</el-button>
        <!-- table展示用户信息 -->
        <el-table style="margin: 10px 0px;" border :data="userArr" @selection-change="selectChange">
            ...
            <el-table-column label="操作" width="300px" align="center">
                <template #="{ row, $index }">
                    ...
                    <el-popconfirm :title="`你确定要删除${row.username}?`" width="260px" @confirm="onDeleteUser(row.id)">
                        <template #reference>
...
const save = async () => {
  ...
  let result: any = await addOrUpdateUser(userParams);
  //添加或者更新成功
  if (result.code == 200) {
      //关闭抽屉
      drawer.value = false;
...
const setRole = async (row: UserData) => {
  ...
  let result: AllRoleResponseData = await getRoleData((userParams.id as number));
  if (result.code == 200) {
      //存储全部的职位
...
const confirmClick = async () => {
  ...
  //分配用户的职位
  let result: any = await userAddRole(data);
  if (result.code == 200) {
      //提示信息
...
const onDeleteUser = async (userId: number) => {
  let result: any = await deleteUser(userId);
  if (result.code == 200) {
...
const onDeleteSelectUser = async () => {
  ...
  //批量删除的请求
  let result: any = await deleteSelectUser(idsList);
